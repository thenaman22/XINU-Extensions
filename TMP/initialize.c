...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	for(i=0; i < 4; i++){
		...
		global_pt = (pt_t *)((FRAME0 + i) * NBPG); 	
		for(j=0; j < 1024; j++){
			global_pt->pt_pres = 1;	
			global_pt->pt_write = 1;
			global_pt->pt_user = 0;
			global_pt->pt_pwt = 0;
			global_pt->pt_pcd = 0;
			global_pt->pt_acc = 0;
			global_pt->pt_dirty = 0;
			global_pt->pt_mbz = 0;
			global_pt->pt_global = 0;
			global_pt->pt_avail = 0;
			global_pt->pt_base = i * FRAME0 + j;	
			global_pt++;		
		}
	}
	init_pd(4, NULLPROC);
	pptr->pdbr = (unsigned long)((FRAME0+4)*NBPG);
	write_cr3(pptr->pdbr);
